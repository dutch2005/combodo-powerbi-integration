...
function registerExtensionContractTests(TestHarness $harness, $root)
{
	...
	$harness->run('module registers the 1.1.1 identity', function () use ($harness) {
		$harness->assertSame(1, count(SetupWebPage::$modules), 'Exactly one module must be registered');
		$harness->assertSame('combodo-powerbi-integration/1.1.1', SetupWebPage::$modules[0][1], 'Registered version must be 1.1.1');
	});
	...
	$harness->run('same-version setup performs no data load', function () use ($harness) {
		XMLDataLoader::reset();
		PowerBiIntegrationInstaller::AfterDatabaseCreation(new Config('EN US'), '1.1.1', '1.1.1');
	...
	$harness->run('upgrade loads the English fallback data once', function () use ($harness) {
		XMLDataLoader::reset();
		PowerBiIntegrationInstaller::AfterDatabaseCreation(new Config('DE DE'), '1.0.1', '1.1.1');
	...
	$harness->run('upgrade loads an existing configured locale file', function () use ($harness) {
		XMLDataLoader::reset();
		PowerBiIntegrationInstaller::AfterDatabaseCreation(new Config('EN US'), '1.0.1', '1.1.1');
	...
	$harness->run('extension manifest declares version 1.1.1', function () use ($harness, $root) {
		...
		$harness->assertSame('1.1.1', (string) $manifest->version, 'Manifest version must be 1.1.1');
	});
